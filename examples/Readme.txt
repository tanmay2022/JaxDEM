...
Yes. I checked the current main branch of JaxDEM before writing this. The current API supports custom Colliders, custom force/energy calculations, JAX integration, and VTK output; the repository explicitly supports 3D simulations and custom modules. 
G
GitHub
+2

For the first ROM, I recommend one 3D spherical Al particle impacting a rigid flat substrate. Cold-spray particle velocities of roughly 450–1000 m/s are representative, and published Al critical velocities are commonly in the several-hundred-m/s range. 
S
ScienceDirect
+1

Below is a complete example. It deliberately keeps the physics simple:

JaxDEM State + System
custom JaxDEM Collider
3D particle
rigid planar substrate at z = 0
capped elastic contact
viscous contact dissipation
kinetic/contact/dissipated/total-energy bookkeeping
deposition criterion
VTK .vtp particle files with energy fields
.pvd time collection for ParaView
no vtk Python dependency; VTP XML is written directly

The custom collider follows the current JaxDEM interface, where compute_force, compute_potential_energy, and create_neighbor_list are the relevant hooks. 
G
GitHub
+1
...
The current JaxDEM examples use the straightforward State.create → System.create → System.step workflow, and its VTK infrastructure writes ParaView-compatible .vtp/.pvd data. 
G
GitHub
+1
...
# Put cold_spray_rom.py in the repository root or examples/
python cold_spray_rom.py


Then open:

output/cold_spray_rom/cold_spray.pvd


in ParaView.

For the particle, apply:
...
What this model is actually doing
...
The damping dissipation is accumulated from
...
Consequently, the useful diagnostic is
...
That energy balance is one of the most useful things to inspect before adding more complicated cold-spray physics.

One important qualification

This is a reduced-order contact model, not a predictive cold-spray plasticity model. In particular, the capped force is an effective nonlinear contact stiffness; the current dissipated_energy is damping dissipation. It should not yet be labelled as true plastic work.

That distinction matters because actual cold-spray deposition depends strongly on particle/substrate material properties, particle size, impact velocity and critical velocity; published Al data show critical velocity can vary substantially with particle size and conditions. 
S
Springer Link
+1

The next upgrade I'd make

The natural next step is to replace the capped spring with a history-dependent elastic–plastic contact law:
...
Then we can run a velocity sweep, e.g. 400–900 m/s, and produce a reduced-order deposition curve comparable to the experimentally observed critical-velocity transition. That would turn this from a contact demo into a useful cold-spray ROM.
